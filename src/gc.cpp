    ...
    #define LUAXC_GC_VALUE_DEFINE_PRIM_BINARY_LOGICAL_OP(NAME, OP)                                           \
        PrimValue NAME(const PrimValue& lhs, const PrimValue& rhs) {                              \
            auto to_bool = [](auto v) -> bool {                                                   \
                ...
                else if constexpr (std::is_same_v<T, Float>) return v != 0.0;                     \
            else if constexpr (std::is_same_v<T, NullObject>) return false;                         \
                else LUAXC_GC_THROW_ERROR("Invalid operand type for boolean coercion");       \
            };                                                                                     \
            return std::visit(detail::overloaded {                                                 \
    ...
    #define LUAXC_GC_VALUE_DEFINE_PRIM_UNARY_BOOL_OP(NAME, EXPR)                                          \
        PrimValue NAME(const PrimValue& val) {                                                 \
            return std::visit([](auto&& v) -> PrimValue {                                      \
                ...
                else if constexpr (std::is_same_v<T, Int>)  return PrimValue::from_bool(EXPR(v != 0)); \
                else if constexpr (std::is_same_v<T, Float>) return PrimValue::from_bool(EXPR(v != 0.0)); \
                else LUAXC_GC_THROW_ERROR("Invalid type for unary bool op")              \
            }, val.get_value());                                                               \
        }
    ...
    #define LUAXC_GC_VALUE_DEFINE_PRIM_UNARY_ARITH_OP(NAME, INT_EXPR, FLOAT_EXPR)                          \
        PrimValue NAME(const PrimValue& val) {                                                  \
            return std::visit([](auto&& v) -> PrimValue {                                       \
                ...
                else if constexpr (std::is_same_v<T, Float>) return PrimValue::from_f64(FLOAT_EXPR(v)); \
                else LUAXC_GC_THROW_ERROR("Invalid type for primitive value")              \
            }, val.get_value());                                                                \
        }
